...
SELECT 
	COUNT(m.id) as Movies_Produced, m.year as Year
FROM movie as m
WHERE m.country = 'USA' OR m.country = 'India'
GROUP BY m.country, m.year
HAVING m.year = 2019;
...
top_director AS (
    ...
    INNER JOIN top_genre ON g.genre = top_genre.genre
    WHERE 
        r.avg_rating > 8
    GROUP BY 
        director_name
...
SELECT * FROM actor_stats LIMIT 1;
...
SELECT 
    ...
    DENSE_RANK() OVER(ORDER BY MAX(avg_rating) DESC) AS actress_rank
FROM names AS n
